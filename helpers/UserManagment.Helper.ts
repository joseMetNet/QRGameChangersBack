...
export const authenticateUser = async (email: String, password: String) => {
    try {
        ...
        const userManagementData = {
            userGroup: process.env.USER_GROUP || 'me_los_tomo',
            userName: email,
            password: password
        };
...
export const createUserInUserManagement = async (email: String, password: String) => {
    try {
        const userManagementUrl = process.env.USER_MANAGEMENT_URL || 'https://usermanagementservicemetnet.azurewebsites.net/UserManagement';
        const userManagementData = {
            userGroup: process.env.USER_GROUP || 'me_los_tomo',
            userName: email,
            password: password
        };
        const userManagementResponse = await axios.post<any>(userManagementUrl, userManagementData);
